...
export class SfpPage implements Page<SfpStatus[]> {
   ...
   */
  load(): Effect.Effect<SfpStatus[], SwOSError> {
    const self = this
    return Effect.gen(function* (_) {
      const response = yield* _(self.client.fetch('/sfp.b'))
      ...
      return yield* _(
        Effect.try(() => {
          ...
          Effect.mapError(
            (e) =>
              new SwOSError(
                `SFP load failed: ${(e as Error).message}\nResponse: ${response || 'N/A'}`
              )
          )
        )
      )
  ...
  save(data: SfpStatus[]): Effect.Effect<void, SwOSError> {
    // SFP page is read-only
    return Effect.fail(new SwOSError('SFP page is read-only'))
  }

  async loadAsync(): Promise<SfpStatus[]> {
    return Effect.runPromise(this.load())
  }

  async saveAsync(data: SfpStatus[]): Promise<void> {
